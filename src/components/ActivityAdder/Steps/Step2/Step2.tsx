...
import {usePropsFilter, useStringFilter} from "../../../../utils/usePropsFilter";
...
export const Step2 = () => {
    ...
    return <div className={cssClasses.selectorWrapper}>
        {editStat ? <EditStat/> :
            <>
                <div className={cssClasses.buttons}><AutoComplete freeSolo={true} label={"Category"} options={ActivityCategory.options}
                              onInputChange={(value) => !value && setCategoryFilter("All")}
                              onActivityChange={(category) => {
                                  setCategoryFilter(category)
                              }}/><Input label="Filter stats" type="text" onChange={(e) => setStatFilter(e)}></Input></div>
                <>
