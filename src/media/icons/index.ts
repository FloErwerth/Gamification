import {ReactComponent as Arrow} from "./arrow.svg";
import {ReactComponent as ArrowFilled} from "./arrow_filled.svg";
import {ReactComponent as Checkmark} from "./Checkmark.svg";
import {ReactComponent as CloseCircle} from "./close_circle.svg";
import {ReactComponent as Close} from "./close.svg";
import {ReactComponent as Controller} from "./controller.svg";
import {ReactComponent as EyeClosed} from "./eyeClosed.svg";
import {ReactComponent as EyeOpened} from "./eyeOpened.svg";
import {ReactComponent as LoginIcon} from "./login.svg";
import {ReactComponent as Settings} from "./settings.svg";
import {ReactComponent as Star} from "./star_yellow.svg";

export {Arrow, ArrowFilled, Checkmark, Close, CloseCircle, Controller, EyeClosed, EyeOpened, LoginIcon, Settings, Star}