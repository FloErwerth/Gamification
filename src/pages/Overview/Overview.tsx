...
export const Overview = () => {
   ...
   const stats = useAppSelector(getActivities).map((stats) => <Stats {...stats} />);
   ...
   return <div>
      ...
      <div className={cssClasses.filterWrapper}>
         <InputWithDelete placeholder={"Filter activities"} type="text" id="Search"
                          customWrapperClasses={css({outlineColor: "rgb(100,200,200)"})} value={filter}
                          onChange={(value) => setFilter(value)}/>
      </div>
      <div className={cssClasses.statsWrapper}>
         {filteredArray}
      </div>
      <div className={cssClasses.activityAdderWrapper}><ActivityAdder/></div>
   </div>
}